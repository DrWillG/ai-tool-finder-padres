...
import { Tool, TOOL_COLORS } from '@/app/data/tools';
...
export default function ToolCard({ tool, index }: ToolCardProps) {
  ...
  const ptLabel: Record<string, string> = { free: 'Gratis', freemium: 'Gratis para empezar', paid: 'De pago' };
        ...
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="font-semibold text-gray-900 text-base leading-tight">{tool.n}</h3>
              <span className={`text-xs font-medium px-2 py-0.5 rounded-full border ${ptColor[tool.pt]}`}>{ptLabel[tool.pt]}</span>
            </div>
            <p className="text-xs text-gray-500 mt-0.5">{tool.c}</p>
          </div>
        </div>
        <p className="text-sm text-gray-600 leading-relaxed flex-1 mb-4">{tool.d}</p>
        <div className="mb-4">
          <SafetyBadge coppa={tool.safety.coppa} adFree={tool.safety.adFree} ageMin={tool.safety.ageMin} buildsSkills={tool.safety.buildsSkills} compact />
        </div>
        ...
        {tool.rv.quote && (
          <div className="mt-3 pt-3 border-t border-gray-50">
            <div className="flex items-center gap-1 mb-1">
              {[...Array(5)].map((_, i) => (
                <svg key={i} className={`w-3 h-3 ${i < Math.round(tool.rv.rating) ? 'text-amber-400' : 'text-gray-200'}`} fill="currentColor" viewBox="0 0 20 20">
            ...
            <p className="text-xs text-gray-500 italic leading-relaxed">&ldquo;{tool.rv.quote}&rdquo;</p>
          </div>
        )}
      </div>
    </motion.div>
